...
function(exports, shader, framebuffer, data) {
	...
	function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
		...
		for (let v = polygon.length; v > 0; v--) {
			let calculatedDerivative = calcDerivative(vertices[polygon[v]], vertices[polygon[v - 1]]);
			
			if (calculatedDerivative !== 0) {
				lastDerivative = calculatedDerivative;
				break;
			}
		}
		...
		for (let v = 0; v < polygon.length; v++) {
			...
			// derivative !== 0)
			if ((derivative + lastDerivative === 0) && lastDerivative !== 0) {
